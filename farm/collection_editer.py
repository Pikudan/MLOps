...
def download_information(farmer_tg_id: int, name_calendar: str):
    ...
    if name_calendar == "problems_for_support":
        info = read_collection_with_composite_filter(name_calendar,[{'atribut': 'status', 'op': '==', 'value': 'open'},
                                                {'atribut': 'user_telegram_id', 'op': '==', 'value': farmer_tg_id}])
    if name_calendar == "records":
        info = read_collection_with_composite_filter(name_calendar,[{'atribut': 'user_telegram_id', 'op': '==', 'value': farmer_tg_id}])    
    doc = []
    for i in info:
        data = i["data"]
